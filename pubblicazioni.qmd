...
# Usiamo il database delle conferenze, non quello delle pubblicazioni
conferenze %>%
  # Se hai una colonna 'year', usala per ordinare, altrimenti ometti arrange
  # arrange(desc(year)) %>% 
  mutate(Entry = paste0("**", event_name, "** (", city, "). *", name, "* [", type, "]")) %>%
  select(Entry) %>%
